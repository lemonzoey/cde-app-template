...
export default {
  ...
  serialize() {
    return {
      cdeAppTemplateViewState: this.cdeAppTemplateView.serialize()
    }
  },
  ...
  addEvent() {
    ...
    iframe.onload = function() {
      ...
      //获取主弹框页面
      let down = document.getElementsByClassName('cde-app-template')[0]
      let mymodal1 =  down.parentNode
      let load = document.getElementsByClassName('selet-list-view')[0]
      let mymodal2 =  load.parentNode
      debugger
  ...
  initCode(add) {
    ...
    if (urlstr == undefined) {
      ...
      atom.confirm({
        message: '未选择项目',
        detailedMessage: '您还未选择需要下载的项目，请选择一个项目后再点击创建按钮'
      })
      return false
    }
    ...
    if (!urlstr.endsWith('.zip')) {
      ...
      atom.confirm({
        message: '链接有误',
        detailedMessage: '您当前选择的下载链接不正确，请联系管理员'
      })
      return false
    }
    ...
    new Promise((resolve, reject) => {
        const stream = request(urlstr)
          .on('error', (error) => {
            setTimeout(() => {
              ...
              atom.confirm({
                message: '文件有问题',
                detailedMessage: '您当前选择的文件有疑问，请联系管理员'
              })
              atom.commands.dispatch(atom.views.getView(atom.workspace), 'cde-app-template:open')
            }, 2000)
        ...
        fs.createReadStream(filepath).pipe(unzip.Extract({
          ...
          .on('error', (err) => {
            atom.confirm({
              message: '解压错误',
              detailedMessage: '您当前下载操作不成功，请检查下您的电脑是否开启了网络代理，如开启了请您关闭后重新尝试下载'
            })
            //关闭loading页面，打开下载页面
            atom.commands.dispatch(atom.views.getView(atom.workspace), 'init:showModal')
            atom.commands.dispatch(atom.views.getView(atom.workspace), 'cde-app-template:open')
            ...
            //压缩文件解压失败，也要删除掉
            let deleteFile2 = rootPath.join(__dirname, filename)
            fs.unlink(deleteFile2)
            console.log("error to unzip");
          }))
      })
      .catch(err => {
        // reject(err)
